...
class MainController extends Controller {
    ...
    {
        $this->email->sender('nebrejaharlyn@gmail.com', 'Embs');
    ...
    Public function uploadFile(){
        // Load the upload library and pass the uploaded file
        $this->call->library('upload', $_FILES["fileToUpload"]);
        
        // Set directory where the file will be uploaded
        $this->upload->set_dir('public');
        
        // Allow multiple file extensions (e.g., jpg, png, pdf, docx)
        $this->upload->allowed_extensions(array('jpg', 'png', 'pdf', 'docx'));
        
        // Allow MIME types corresponding to the file extensions
        $this->upload->allowed_mimes(array('image/jpeg', 'image/png', 'application/pdf', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'));
        
        // Check if the uploaded file is an image (optional)
        // Remove the is_image() check if you want to accept non-image files as well
        // $this->upload->is_image(); // Uncomment if you only want to allow images
        ...
        if($this->upload->do_upload()) {
            ...
            // If upload fails, get the errors and display them
